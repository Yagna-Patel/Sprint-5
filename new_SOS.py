...
class gui (board): # this will make it so inhertis the board for access.
    ...
    def start_new_game(self):
        ...
        if not self.is_replaying:  # we only wipe if we're not replaying
    ...
    def computer_turn(self):
        # fix for a bug i ran into. 
        # probably not the best way to handle the bug
        if self.is_replaying:  # replaying so don't let computer make moves
            return
